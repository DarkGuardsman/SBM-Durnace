...
public class TileDurnace extends TileEntityFurnace {
    ...
    public void update() {
        ...
        if (wasBurning && !isBurning()) world.setBlockState(pos, Registries.durnace.getDefaultState().withProperty(BlockDurnace.FACING, world.getBlockState(pos).getValue(BlockDurnace.FACING)));
    }
    ...
    protected void burnFuel(ItemStack fuel, boolean burnedThisTick) {
        ...
        if (this.isBurning()) {
            ...
            if (!burnedThisTick) world.setBlockState(pos, Registries.lit_durnace.getDefaultState().withProperty(BlockDurnace.FACING, world.getBlockState(pos).getValue(BlockDurnace.FACING)));
        }
    }
    ...
    public boolean shouldRefresh(World world, BlockPos pos, IBlockState oldState, IBlockState newState) {
        if (oldState.getBlock() == Registries.durnace && newState.getBlock() == Registries.lit_durnace) return false;
        else if (oldState.getBlock() == Registries.lit_durnace && newState.getBlock() == Registries.durnace) return false;
        return true;
    }
}
